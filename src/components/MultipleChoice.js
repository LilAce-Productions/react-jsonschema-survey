...
class MultipleChoice extends React.Component {
  ...
  onChangeForm(e) {
    ...
    this.props.onSaveAnswer({
      [`multipleChoice_questionNumber_${questionNumber}`]: questionNumber,
      [`multipleChoice_questionTitle_${questionNumber}`]: questionTitle,
      [`multipleChoice_questionType_${questionNumber}`]: questionType,
      [`multipleChoice_questionAnswer_${questionNumber}`]: e.formData
    });
  }
  handleQuestionChange(e) {
    const val = e.target.value;
    const rest = this.state.uiSchema.multipleChoice;
    this.setState({ uiSchema: Object.assign({}, this.state.uiSchema, {
      multipleChoice: { ...rest, 'ui:title': val }
    }) }, () => this.updateQuestionArray());
  }
  reorderLabel(index, newIndex) {
    const arr = this.state.schema.properties.multipleChoice.enum;
    [arr[index], arr[newIndex]] = [arr[newIndex], arr[index]];
    let sch = null;
    for (let i = 0; i < arr.length; i++) {
      sch = update(this.state.schema, { properties: { multipleChoice: { enum: { [i]: { $set: arr[i] } } } } });
    }
  ...
  addChoice(prevChoiceText) {
    ...
    if (val) {
      const choiceArray = this.state.schema.properties.multipleChoice.enum;
      if (choiceArray && choiceArray.length > 0 && !choiceArray.includes(val)) {
        let sch = null;
        if (prevChoiceText) {
          for (let i = 0; i < choiceArray.length; i++) {
            if (choiceArray[i] === prevChoiceText) {
              sch = update(this.state.schema, { properties: { multipleChoice: { type: { $set: 'string' }, enum: { [i]: { $set: val } } } } });
              break;
            }
          }
        } else {
  ...
  removeChoice(prevChoiceText) {
    ...
    if (val) {
      ...
      if (choiceArray && choiceArray.length > 0 && choiceArray.includes(val)) {
        ...
        const sch = update(this.state.schema, { properties: { multipleChoice: { type: { $set: 'string' }, enum: { $splice: [[index, 1]] } } } });
        this.setState({
          schema: sch,
          choiceText: '',
  ...
  handleChoiceText(e) {
    const val = e.target.value;
    this.setState({ choiceText: val }, () => this.updateQuestionArray());
  }
  saveQuestion() {
